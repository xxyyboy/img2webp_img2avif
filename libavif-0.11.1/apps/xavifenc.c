...
int apng2avif(const char *apng_file, const char *avif_file, avifEncoder *encoder ,avifPixelFormat yuvFormat, uint32_t depth)
{
    ...
    const AVCodec * apng_decoder = avcodec_find_decoder(format_ctx->streams[video_stream_index]->codecpar->codec_id);
    ...
    avifEncoder * avif_encoder = encoder;
    encoder->timescale = timescale;

    printf("Input File Format:%s \n", avifPixelFormatToString(apng_decoder_ctx->pix_fmt));
        ...
        if (pkt.stream_index == video_stream_index) {
            ...
            if (frame) {
                ...
                for (int plane = 0; plane < 3; ++plane) {
                    ...
                    int plane_width = (plane == 0) ? yuv_frame->width : (yuv_frame->width + 1) / 2;
                    int plane_height = (plane == 0) ? yuv_frame->height : (yuv_frame->height + 1) / 2;

                    for (int y = 0; y < plane_height; ++y) {
                        memcpy(dst, src, plane_width);
                        src += src_stride;
                        dst += dst_stride;
                    }
                }
                ...
                if (apng_decoder_ctx->pix_fmt == AV_PIX_FMT_RGBA) {
                    ...
                    for (int y = 0; y < yuv_frame->height; ++y) {
                        av_image_copy_plane(avif_image->alphaPlane + y * avif_image->alphaRowBytes,
                                            avif_image->alphaRowBytes,
                                            yuv_frame->data[3] + y * yuv_frame->linesize[3],
                                            yuv_frame->linesize[3],
                                            yuv_frame->width,
                                            1);
                    }
                }
    ...
    FILE * output_file = fopen(avif_file, "wb");
...
static char in_file_path[MAX_PATH] = {0};
static char temp_file_path[MAX_PATH] = {0};
static char out_file_path[MAX_PATH] = {0};
...
/*stat file*/
int statfile(const char *in_file, const char *out_file)
{
  /*  判断in_file out_file:
...
static void syntax(void)
{
    ...
    printf("    --min Q                           : Set min quantizer for color (%d-%d, where %d is lossless. default:24)\n",
           AVIF_QUANTIZER_BEST_QUALITY,
           AVIF_QUANTIZER_WORST_QUALITY,
           AVIF_QUANTIZER_LOSSLESS);
    printf("    --max Q                           : Set max quantizer for color (%d-%d, where %d is lossless. default:26)\n",
           AVIF_QUANTIZER_BEST_QUALITY,
           AVIF_QUANTIZER_WORST_QUALITY,
           AVIF_QUANTIZER_LOSSLESS);
    ...
    printf("    -s,--speed S                      : Encoder speed (%d-%d, slowest-fastest, 'default' or 'd' for codec internal defaults. default speed: 9)\n",
           AVIF_SPEED_SLOWEST,
           AVIF_SPEED_FASTEST);
    ...
    printf("    --skipapng                        : skip apng conversion.\n");
    ...
    avifPrintVersions();
    printf("version:xavifenc-20230818\n");
}
...
int main(int argc, char * argv[])
{
    ...
    while (argIndex < argc) {
        ...
        if (!strcmp(arg, "--")) {
            ...
            avifPrintVersions();
            printf("version:xavifenc-20230727\n");
            goto cleanup;
        ...
        } else if (!strcmp(arg, "--skipapng"))  {
            skipapng=1;
        } else if (!strcmp(arg, "--forcesmall"))  {
            forcesmall=1;
        } else if (!strcmp(arg, "--retry")) {
            NEXTARG();
            retry = atoi(arg);
        }  else if (arg[0] == '-') {
    ...
    /*判断in_file outputFilename:
      1:新in_file outputFilename 或 outputFilename 转换 或 不转换
      2:老的in_file outputFilename : 不转换，直接返回 退出
      3:新的in_file outputFilename : 转换
      4:至少有一个源文件需要转换
    */

    const char * in_file = NULL;
    if(0 < input.filesCount){
        in_file = input.files[0].filename;
        if (0 != statfile(in_file, outputFilename))
        {
            return -1;
        }
        else{
            outputFilename = temp_file_p; //point to temp file
        }
    }
    ...
    printf("Encoding with AV1 codec '%s' timescale[%d], speed [%d], color QP [%d (%s) <-> %d (%s)], alpha QP [%d (%s) <-> %d (%s)], %s, %d worker thread(s), please wait...\n",
    ...
    if(NULL != in_file){
        ...
        if (0 == strcmp("apng", format_ctx->iformat->name) && 0 == skipapng)
        { // apng
            if (0 != apng2avif(in_file, outputFilename, encoder, image->yuvFormat , sourceDepth))
            {
                printf("Failed to convert APNG to AVIF.\n");
                returnCode = -1;
                goto Error;
            }

            avformat_close_input(&format_ctx);
            printf("Succeeded to convert APNG to AVIF.\n");
            returnCode = 0;
            goto Apng;
        }
        else if(0 == strcmp("apng", format_ctx->iformat->name) && 1 == skipapng){
            printf("Skip APNG to AVIF.\n");
            returnCode = 0;
            goto Apng;
        }
    }
    ...
    if (fwrite(raw.data, 1, raw.size, f) != raw.size) {
        ...
        printf("Wrote AVIF: %s successfully.\n", outputFilename);
    }
    fclose(f);
