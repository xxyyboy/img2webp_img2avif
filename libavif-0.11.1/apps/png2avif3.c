...
int main(int argc, char * argv[])
{
    ...
    AVCodec * output_codec = avcodec_find_encoder_by_name("libaom-av1"); //AV_CODEC_ID_AV1 libx265
    if (!output_codec) {
    ...
    //output_codec_ctx->codec_id = AV_CODEC_ID_AV1; //AV_CODEC_ID_AV1  AV_CODEC_ID_AV1 AV_CODEC_ID_HEVC
    //output_codec_ctx->codec_tag = AV_RL32("av01"); // AV_RL32("avis")
    ...
    //output_codec_ctx->max_b_frames = 0;
    output_codec_ctx->pix_fmt = AV_PIX_FMT_YUV420P;
    output_codec_ctx->color_range = AVCOL_RANGE_JPEG; // 设置色彩范围为 JPEG (全范围)
    // 设置编码器选项
    av_opt_set(output_codec_ctx->priv_data, "crf", "10", 0); // 设置 CRF 值，范围从 0 到 63，值越低质量越高
    av_opt_set(output_codec_ctx->priv_data, "preset", "slow", 0); // 设置编码速度，可选值：ultrafast、superfast、veryfast、faster、fast、medium、slow、slower、veryslow
    ...
    while (1) {
        int ret = av_seek_frame(input_format_ctx, video_stream_index, current_pts, AVSEEK_FLAG_FRAME);
        if (ret < 0) {
            break;
        }
    ...
    while (1) {
        int ret = avcodec_receive_packet(output_codec_ctx, &packet);
        if (ret == AVERROR_EOF) {
            break;
