...
int main(int argc, char * argv[])
{
    ...
    avif_encoder->minQuantizer = 24;
    avif_encoder->maxQuantizer = 26;
    ...
    avif_encoder->speed = 10;

    // Set output color range and color primaries
    apng_decoder_ctx->color_range = AVIF_RANGE_FULL;
    apng_decoder_ctx->color_primaries = AVIF_COLOR_PRIMARIES_BT709;
        ...
        if (pkt.stream_index == video_stream_index) {
            ...
            if (frame) {
                ...
                if (apng_decoder_ctx->pix_fmt != AV_PIX_FMT_RGBA) {
                    avif_image->alphaPlane = NULL;
                    avif_image->alphaRowBytes = 0;
                }
				...
				avif_image->colorPrimaries = colorPrimaries;
				avif_image->transferCharacteristics = transferCharacteristics;
				avif_image->matrixCoefficients = matrixCoefficients;
				avif_image->yuvRange = AVIF_RANGE_FULL;
				avif_image->alphaPremultiplied = AVIF_FALSE;
                avif_image->depth = 8;
                avif_image->width = yuv_frame->width;
                avif_image->height = yuv_frame->height;

                uint32_t sourceDepth = 0;
                avifAppSourceTiming firstSourceTiming;
                struct y4mFrameIterator * frameIter;
                const  avifAppFileFormat nextInputFormat = avifReadImage(argv[1],AVIF_PIXEL_FORMAT_NONE,0,chromaDownsampling,AVIF_FALSE,AVIF_FALSE,AVIF_FALSE, avif_image, NULL, &firstSourceTiming, &frameIter);
