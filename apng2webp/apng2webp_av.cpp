...
int file_convert(const char* in_file, const char* out_file){
    AVFormatContext* format_ctx = nullptr;
    if (avformat_open_input(&format_ctx, in_file, nullptr, nullptr) != 0) {
        return -1;
    }
    ...
    if (0 == strcmp("apng", format_ctx->iformat->name)) { //apng
        if (!apng_to_webp(in_file, out_file)) {
            cout << "Failed to convert APNG to WEBP." << endl; 
            ret = -1; 
        }
    }
    else if (0 == strcmp("gif", format_ctx->iformat->name)) { //gif
        cout << "Failed, the file type is:" << format_ctx->iformat->name <<endl;
        ret = -1;
    }
...
int main(int argc, char* argv[]) {
    ...
    if (0 != file_convert(argv[1], argv[2])) {
        cout << "Failed, Only support APNG to WEBP." << endl; 
        return -1; 
    }
