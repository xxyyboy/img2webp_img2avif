...
/////
    ...
    std::vector<uint8_t> background_rgba(width * height * 4, 0);
    std::vector<uint8_t> frame_rgba(width * height * 4, 0);
    std::vector<uint8_t> previous_rgba(width * height * 4, 0);
    ...
    for (png_uint_32 frame_num = 0; frame_num < num_frames; ++frame_num) {
        ...
        if (delay_den == 0) {
            delay_den = 100; // Assign a default value if denominator is zero
        }

        // Store the current frame for future dispose operation
        if (dispose_op == PNG_DISPOSE_OP_PREVIOUS) {
            previous_rgba = background_rgba;
        }
        ...
        frame.width = width;
        frame.height = height;
        frame.duration = delay_num * 1000 / delay_den;  // Convert to milliseconds
        frame.rgba.resize(width * height * 4, 0);

        std::vector<png_bytep> row_pointers(sub_height);
        for (png_uint_32 y = 0; y < sub_height; ++y) {
            row_pointers[y] = frame_rgba.data() + (y * width * 4) + x_offset * 4;
        }

        png_read_image(png_ptr, row_pointers.data());

        // Blend the current frame with the background
        for (png_uint_32 y = y_offset; y < y_offset + sub_height; ++y) {
            for (png_uint_32 x = x_offset; x < x_offset + sub_width; ++x) {
                uint8_t* src_pixel = &frame_rgba[((y - y_offset) * sub_width + (x - x_offset)) * 4];
                uint8_t* dst_pixel = &background_rgba[(y * width + x) * 4];
                ...
                if (blend_op == PNG_BLEND_OP_SOURCE) {
                    ...
                    int out_alpha = src_alpha + dst_alpha * (255 - src_alpha) / 255;

                    if (out_alpha != 0) {
                        for (int c = 0; c < 3; ++c) {
                            dst_pixel[c] = (src_pixel[c] * src_alpha + dst_pixel[c] * dst_alpha * (255 - src_alpha) / 255) / out_alpha;
                        }
                    }
