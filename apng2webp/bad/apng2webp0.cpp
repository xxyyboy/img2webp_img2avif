...
int             tlevel, bcolor, back_r, back_g, back_b, loop;
...
void apng_to_webp(std::vector<APNGFrame>& frames, unsigned char * pAGIF) {
    WebPMux* mux = NULL;
    WebPData webp_data;
...
int main(int argc, char** argv)
{
  ...
  {
    printf("Usage : apng2gif anim.png [anim.gif] [-t tlevel] [-b bcolor] [-l loop]\n\n"
           "tlevel: transparency threshold level (format: -t 128)\n"
           "bcolor: background blend color (format: -b \"#808080\" or -b 128 128 128)\n"
           "loop: the number of loops, 0 for infinite loop, \"orig\" set loops number as specified by apng itself (format: -l 0)"
           "When -b is used, -t is ignored. Default is -t 128 -l 0, no bcolor.\n");
    return 1;
  }
  ...
  if (loop >= 0) {
      ...
      printf("use original loop times: %d\n", num_loops);
  }
